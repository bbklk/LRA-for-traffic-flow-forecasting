...
def realtime_batch_train(file_name,model_path):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        # loss = graph.get_operation_by_name('traffic_model/truediv').outputs[0] #traffic_model/div_1
        loss = graph.get_operation_by_name('traffic_model/Mean').outputs[0]  # traffic_model/div_1
        train_op = graph.get_operation_by_name('traffic_model/Adam')

        ########################realtime_batch_train#################################
        ...
        #print(test_batch_data)
        test_date, test_traffic_input, test_targets = prepare_feed_data(test_batch_data)
        
        #saver = tf.train.Saver()
        for i in range(EPOCH_NUM):#EPOCH_NUM
            test_loss, _ = sess.run([loss, train_op],
                                                feed_dict={traffic_flow_input_data: test_traffic_input,
                                                           targets: test_targets,learning_rate:0.0001,
                                                           is_training: True})
            print("test loss ",test_loss)
        # test phase
        test_predicts = sess.run(predict,feed_dict={traffic_flow_input_data: test_traffic_input,
                                           is_training: False})
        # reverse to normal scale
